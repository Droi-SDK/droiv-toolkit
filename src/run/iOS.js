...
function installDep({
  ...
  return utils.exec('pod update').then(() => ({
    xcodeProject,
    options,
    rootPath
  }));
...
function findIOSDevice({
  ...
  return new Promise((resolve, reject) => {
    let deviceInfo = '';
    try {
      deviceInfo = child_process.execSync('xcrun instruments -s devices', {
        encoding: 'utf8'
      });
    } catch (e) {
      reject(e);
...
function _runAppOnSimulator({
  ...
  let simctlInfo = '';
  try {
    simctlInfo = child_process.execSync('xcrun simctl list --json devices', {
      encoding: 'utf8'
    });
  } catch (e) {
    reject(e);
  ...
  try {
    child_process.execSync(`xcrun instruments -w ${device.udid}`, {
      encoding: 'utf8'
    });
  } catch (e) {
  ...
  try {
    child_process.execSync(`xcrun simctl install booted ${appPath}`, {
      encoding: 'utf8'
    });
  } catch (e) {
    reject(e);
  ...
  try {
    child_process.execSync(`xcrun simctl launch booted ${bundleID}`, {
      encoding: 'utf8'
    });
  } catch (e) {
    reject(e);
...
function simulatorIsAvailable(info, device) {
  info = info.devices;
  var simList = info['iOS ' + device.version];
...
function _runAppOnDevice({
  ...
  try {
    if (!fs.existsSync(appPath)) {
      ...
      child_process.execSync(path.join(__dirname, '../build/lib/cocoapods-build') + ' . Debug', {
        encoding: 'utf8'
      });
    }

    console.log(child_process.execSync(`../../node_modules/.bin/ios-deploy --justlaunch --debug --id ${deviceId} --bundle ${path.resolve(appPath)}`, {
      encoding: 'utf8'
    }));
  } catch (e) {
    reject(e);
